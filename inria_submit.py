...
def main():
    ...
    TT.restore_snapshot(model, None, auto_file(args.checkpoint))
    ...
    with torch.no_grad():
        ...
        for test_fname in tqdm(x, total=len(x)):
            ...
            slicer = ImageSlicer(image.shape, args.patch_size, args.patch_size // 2, weight='pyramid')
            patches = slicer.split(image)

            patches = list(aug.tta_d4_aug(patches))
            testset = InMemoryDataset(patches, None, transform=test_transform)
            trainloader = DataLoader(testset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True, drop_last=False)
            ...
            items.append((patches_pred, slicer, os.path.join(experiment_dir, basename + '.tif')))

        with Pool(12) as wp:
            wp.imap_unordered(merge_and_save, items)


def merge_and_save(args):
    patches_pred, slicer, output_directory = args

    patches_pred = list(aug.tta_d4_deaug(patches_pred))
