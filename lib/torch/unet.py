...
class _FactorizedConv3BN(nn.Module):
    def __init__(self, in_: int, out: int, bn=False):
        super().__init__()
...
class _FactorizedUNetModule(nn.Module):
    def __init__(self, in_: int, out: int):
        super().__init__()
        self.l1 = _FactorizedConv3BN(in_, out)
        self.l2 = _FactorizedConv3BN(out, out)
...
class FactorizedUNet(nn.Module):
    output_downscaled = 1
    module = _FactorizedUNetModule
    ...
    def __init__(self,
                 ...
                 num_classes=1,
                 add_output=True):
        super(FactorizedUNet, self).__init__()
        self.num_classes = num_classes
        assert len(down_filter_factors) == len(up_filter_factors)
        assert down_filter_factors[-1] == up_filter_factors[-1]
        down_filter_sizes = [filters_base * s for s in down_filter_factors]
