...
class CapsuleLayer(nn.Module):
    def __init__(self, mode, input_channels, kernel_size, stride, num_capsules, capsule_dims, routing):
        ...
        self.one_kernel = torch.ones([1, 1, kernel_size, kernel_size])

        if mode == 'conv':
            pad = kernel_size // 2
            self.convOp = nn.Conv2d(input_channels, capsule_dims * num_capsules, kernel_size=kernel_size, stride=stride, padding=pad)
            self.maxpool = nn.Sequential(nn.ReplicationPad2d(pad), nn.MaxPool2d(kernel_size=kernel_size, stride=1))
            ...
            self.maxpool = nn.Sequential(
                ...
                nn.MaxPool2d(kernel_size=kernel_size, stride=1))
    ...
    def forward(self, x: torch.autograd.Variable):
        ...
        u_t_list = [torch.squeeze(u_t, 1) for u_t in torch.split(x, 1, dim=1)]  # Splits into t_0 chunks
        u_hat_t_list = []
        for u_t in u_t_list:
            # u_t: [N, z_0, H_0, W_0]
        ...
        for d in range(self.routing):
            ...
            for b_t, u_hat_t in zip(b_t_list, u_hat_t_list_):
                ...
                # print('maxpool', b_t.size(), b_t_max.size())
...
class SegCaps(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3):
        ...
        # self.dec31 = CapsuleLayer('up', input_channels=32, capsule_dims=32, kernel_size=5, stride=1, num_capsules=8, routing=3)
        ...
        # self.dec21 = CapsuleLayer('up', input_channels=32, capsule_dims=16, kernel_size=5, stride=1, num_capsules=4, routing=3)
        ...
        # self.dec11 = CapsuleLayer('up', input_channels=16, capsule_dims=16, kernel_size=5, stride=1, num_capsules=2, routing=3)
...
def compute_loss(v_lens, recons, images, labels):
    total_loss = class_loss(v_lens, labels) + 0.0005 * reconstruction_loss(recons, images)
    return total_loss
